import { SiteContent, Plan, Testimonial, Brand, Metric, Pillar } from '../hooks/useSiteContent';
...
export default function LivePreview({ content, plans, testimonials, brands, metrics, pillars }: Props) {
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-hairline" style={{ transform: 'scale(0.55)', transformOrigin: 'top center', width: '182%', marginLeft: '-41%' }}>
          ...
          <div className="border-b border-hairline px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-sm tracking-tight">{content.header_brand_name}<span className="text-silver-400">.</span> <span className="text-[10px] text-silver-500 uppercase tracking-wider">{content.header_brand_suffix}</span></span>
            </div>
            <div className="bg-ink text-white px-4 py-2 rounded-full text-xs font-medium">{content.header_cta}</div>
          </div>
          ...
          <div className="px-10 py-16 bg-white">
            ...
            <h1 className="text-4xl font-medium tracking-tight leading-[0.95]">
              {content.hero_title_1}
            </h1>
            <h1 className="text-4xl font-medium tracking-tight leading-[0.95] mt-1" style={{ background: 'linear-gradient(180deg, #233d68 0%, #1a2b4a 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            ...
            <div className="mt-6 inline-flex items-center gap-2 bg-ink text-white px-5 py-3 rounded-full text-xs font-medium">
              {content.hero_cta_primary}
            </div>
          </div>

          {/* Marquee Preview */}
          <div className="border-y border-hairline bg-bone py-4 px-6 flex gap-6 overflow-hidden">
          ...
          {/* Methodology Preview */}
          <div className="py-12 px-10 bg-bone border-y border-hairline">
            <div className="text-[10px] uppercase tracking-wider text-silver-600">{content.methodology_label}</div>
            <h2 className="mt-3 text-xl font-medium tracking-tight" dangerouslySetInnerHTML={{ __html: content.methodology_title }} />
            <div className="mt-8 grid grid-cols-2 gap-3">
              {pillars.map(p => (
                <div key={p.id} className="bg-white rounded-xl p-4 border border-hairline">
                  <div className="flex justify-between items-start">
                    <div className="w-8 h-8 rounded-lg bg-ink" />
            ...
            <div className="mt-6 grid grid-cols-3 gap-4">
              {founderStats.map((s, i) => (
                <div key={i} className="border-t border-hairline pt-3">
                  <div className="text-lg font-medium">{s.number}</div>
          ...
          {/* Metrics Preview */}
          <div className="py-12 px-10 bg-ink text-white">
          ...
          <div className="py-12 px-10 bg-bone border-y border-hairline">
            ...
            <div className="mt-8 grid grid-cols-3 gap-3">
              {plans.map(p => (
                <div key={p.id} className={`rounded-xl p-4 ${p.featured ? 'bg-ink text-white' : 'bg-white border border-hairline'}`}>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              {testimonials.map(t => (
                <div key={t.id} className="bg-white rounded-xl p-4 border border-hairline">
                  <p className="text-[11px] text-ink leading-relaxed">"{t.quote}"</p>
          ...
          <div className="py-12 px-10">
            <div className="bg-ink text-white rounded-2xl p-8">
              ...
              <div className="mt-5 inline-flex items-center gap-2 bg-white text-ink px-4 py-2 rounded-full text-xs font-medium">{content.cta_button}</div>
            </div>
          </div>

          {/* Footer Preview */}
          <div className="border-t border-hairline py-6 px-10 flex items-center justify-between text-[10px] text-silver-500">
            <span className="font-medium text-ink">{content.footer_brand}<span className="text-silver-400">.</span> {content.footer_suffix}</span>
